...
## Agreed interaction boundaries — observation-only teaching and voice guidance
...
Founder clarification E9 (2026-09-26) makes this restriction authoritative for
every Tro path, including F11. A user-initiated voice or text instruction starts
a bounded guidance-planning run for the single external window pinned when the
instruction begins. Instructor Cursor tools stage visual demonstrations only;
they cannot dispatch native input. Cancel/emergency stop never replays interrupted
capture, transcription, planning or presentation work.
...
| C03 | Changing a URL requires walking around to enter it; wants one button for the action. E1; E2 01:25–01:26, 01:30–01:31                    | Teacher distributes a URL/resource and Tro may show the learner where to open it; the learner performs the external-app action | Delivery and observed opening remain distinct; Tro never launches, switches or navigates another app on the learner's behalf                                                                    | P4, P7-A                            |
...
| C09 | Quiet voice commands may fail and need repetition. E1; E2 supports the noisy environment, not an exact quote                            | Exactly-two-modifier push-to-talk, rolling completed-file transcription, visible local state, and complete text/button paths   | Partial/failed/empty transcription never dispatches; release starts at most one guidance plan; the same observation-only journey works with microphone disabled                                 | P2 silent path, P7-A voice/guidance |
...
| Computer interaction | CUA Driver Python SDK                          | Read-only selected-window observation for F10 and F11; no native input authority                                    |
| Teaching cursor      | Agents SDK tools + transparent SVG/CSS overlay | Staged highlighting, pointing, explanations and local ghost trajectories                                            |
...
CUA supplies read-only observation to F10 and F11 teaching. The Python runtime
owns an explicit Instructor Cursor tool list that stages point, ghost-click,
ghost-drag, type and scroll demonstrations for one pinned `(pid, window_id)`.
It has no input adapter and cannot launch or focus apps, move the pointer, click,
drag, type, scroll, use clipboard/shell/filesystem, or dispatch generic native
commands. React receives projections only. Interrupted plans and audio are
...
| P7-A  | Exactly-two-key voice/text observation-only guidance             | P1 native/CUA; P2 runtime       | Revised by E9/F11; source, packaged native, latency and cost acceptance all required                                                                                |
...
| P7-A: voice + visual guidance         | Add exactly Command+Control (macOS) / LeftCtrl+LeftAlt (Windows) hold-to-talk, rolling completed-file transcription, final-only dispatch, text fallback and one-window Instructor Cursor guidance | One simple walkthrough begins after release; partial/failed speech never plans or presents                      |
...
### P7-A — Exactly-two-key voice and observation-only guidance
...
- [ ] Provide complete text fallback through the same preparation, planning,
      presentation, cancellation and evidence path.
...
- [ ] Measure chord-to-HUD, release-to-final, final-to-dispatch, first-cue latency,
...
Exit: the microphone-disabled path and packaged macOS/Windows chord paths show
one harmless selected-window walkthrough without moving the pointer or delivering
input; emergency stop/auth loss/target loss cancels without replay. If latency, noisy
...
- [ ] On signed macOS arm64 and Windows x64 packages, verify the passive global
      chord hooks, microphone/screen permissions, AltGr rejection, pinned-window
      read-only policy, click-through overlay, cancellation and P7-A latency/cost evidence.
...
| D05 | Explanation/answer-reveal policy and interruption                                                     | F10 governs guided teaching; F11 is a voice/text input to the same observation-only journey. Neither changes answer-reveal policy | P3 instructional policy; F11 never bypasses lesson policy    |
...
## Observation-only Instructor Cursor correction record (2026-09-26)

The founder clarified that the separate teaching cursor may expose an extensible,
closed list of `show_student_*` tools, but those tools only point, circle, animate
ghost trajectories and explain. The learner performs every real external-app
action. This E9 decision supersedes the E8/F11 direct-control exception above;
the E8 text remains solely as historical context for the retained push-to-talk,
rolling completed-file transcription and final-only dispatch decisions.

F11 now starts one bounded guidance plan for the pinned window. Python tool
handlers stage at most three validated `PlannedStep` values without CUA, UI,
network or persistence side effects. After the whole plan is accepted, Rust and
React present the current cue in a click-through overlay. Fresh read-only
observation or explicit learner Continue advances the journey. Tool invocation,
