...
## Agreed interaction boundaries — guided teaching and bounded voice control

Founder clarification, 2026-09-13 (F10): in guided teaching mode Tro shows where and how to click,
...
This historical restriction remains the authority for guided teaching. Founder
clarification E8 (2026-09-25) supersedes it only for F11: a user-initiated voice
or text instruction may run a bounded computer-use loop in the single external
window pinned when the instruction begins. The loop may use allowlisted native
input, but cannot launch or switch applications, use a shell or clipboard, or
cross the pinned-window boundary. Consequential or ambiguous actions pause for
explicit approval in Tro, and cancel/emergency stop never replay interrupted work.

F10 otherwise concerns external-app/device control; ordinary explicit Tro
...
| E9  | Founder clarification in this task, 2026-09-26                                                                                  | English and Vietnamese are application-interface choices. Selecting Vietnamese localizes the complete Tro interface and is independent of voice transcription and lesson/guidance language.                                                                                                            |
...
| C03 | Changing a URL requires walking around to enter it; wants one button for the action. E1; E2 01:25–01:26, 01:30–01:31                    | Teacher distributes a URL/resource; guided mode leaves opening to the learner, while an explicit F11 instruction may act only in the already-pinned window | Delivery and observed opening remain distinct; F11 cannot launch or switch apps and consequential navigation requires confirmation                                                              | P4, P7-A                           |
...
| C09 | Quiet voice commands may fail and need repetition. E1; E2 supports the noisy environment, not an exact quote                            | Exactly-two-modifier push-to-talk, rolling completed-file transcription, visible local state, and complete text/button paths                               | Partial/failed/empty transcription never dispatches; release dispatches at most one final instruction; the same bounded journey works with microphone disabled                                  | P2 silent path, P7-A voice/control |
...
| --- | ------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------- |
| F01 | OpenAI Agents SDK controls CUA Driver. E4/E8                                                                                         | Python runtime owns orchestration; F10 teaching uses read-only observation, while F11 gives a run-scoped ComputerTool adapter only the selected-window actions allowed by native policy                                                                                                                                    | P1, P2, P7-A     |
...
| F10 | In guided teaching, show where/how to click, drag, scroll or type; the learner performs it. E6                                       | Read-only CUA, visual gesture cues, learner action then fresh check; this historical boundary remains active outside the explicit F11 control mode                                                                                                                                                                         | P1 onward        |
| F11 | User-initiated, selected-window, bounded computer control by exactly-two-key push-to-talk or text fallback. E8                       | Hold Command+Control on macOS or LeftCtrl+LeftAlt on Windows; Rust captures bounded in-memory audio and rolling completed WAV files, Python runs one bounded ComputerTool loop, consequential actions require main-window confirmation, and stop/auth/target loss cancels with no replay                                   | P7-A, P8         |
...
| Computer interaction | CUA Driver Python SDK                          | F10 read-only observation plus F11 run-scoped selected-window input behind a closed policy and confirmation boundary |
...
Each guided teaching step identifies its objective, explanation, observed target,
actor (the learner in F10 mode), and evidence required to continue.
...
CUA supplies read-only observation to F10 teaching. F11 has a separate bounded
adapter implementing only screenshot, click, drag, move, scroll, keypress and
typing for one pinned `(pid, window_id)`. The Python action loop owns that
adapter; React receives projections only. Each mutation revalidates target and
geometry, applies action/deadline budgets, and pauses consequential or ambiguous
work for a host confirmation. App launch, arbitrary focus, clipboard, shell,
filesystem and generic native dispatch remain unavailable. Interrupted action
runs and audio are memory-only and are never replayed.
...
| P7-A  | Exactly-two-key voice/text bounded computer control              | P1 native/CUA; P2 runtime       | Planned by E8/F11; source, packaged native, latency and cost acceptance all required                                                                                |
| P7-B  | Evidence-based classroom summaries                               | P5/P6; D03                      | Planned                                                                                                                                                             |
| P8    | Signed native acceptance, upgrade, real pilot and replacement    | Applicable P1–P7 gates          | Planned                                                                                                                                                             |
...
| P4: resource guidance                 | Add selected-recipient resources/instructions and separate delivery/learner follow-through receipts                                                                                                                                       | Resource delivery does not itself invoke F11; learner follow-through stays distinct                             |
...
| P7-A: voice + bounded control         | Add exactly Command+Control (macOS) / LeftCtrl+LeftAlt (Windows) hold-to-talk, rolling completed-file transcription, final-only dispatch, text fallback, one-window ComputerTool execution and explicit consequential-action confirmation | Safe actions begin after release without extra clicks; partial/failed speech never acts                         |
...
### P7-A — Exactly-two-key voice and bounded computer control

- [ ] Hold exactly Command+Control on macOS or LeftCtrl+LeftAlt on Windows;
      show local listening state immediately and release either key to finalize.
...
- [ ] Measure chord-to-HUD, release-to-final, final-to-dispatch, action latency,
      billed audio and cost. Target p95 50 ms local listening, p95 1,200 ms
      release-to-final and p95 50 ms final-to-dispatch on the reference network.

Dependencies: P1 native/CUA foundation and P2 runtime; authenticated provider
access from P3 may be developed independently. Covers C09/F01/F11.
Exit: the microphone-disabled path and packaged macOS/Windows chord paths perform
one harmless selected-window task; risky work pauses for explicit approval;
emergency stop/auth loss/target loss cancels without replay. If latency, noisy
accuracy, signing or clean-device evidence is unavailable, release acceptance stays open.

### P7-B — Evidence-based classroom summaries
...
Dependencies: P5/P6; D03 report scope and D07 pilot metrics. Covers C06/C10.
Exit: every progress claim can be traced to allowed evidence; no inferred
ability ranking or unproven mastery.
...
- [ ] On signed macOS arm64 and Windows x64 packages, verify the passive global
      chord hooks, microphone/input permissions, AltGr rejection, pinned-window
      action policy, confirmation, cancellation and the P7-A latency/cost evidence.
...
| D05 | Explanation/answer-reveal policy and interruption                                                     | F10 governs guided teaching; F11 separately authorizes user-initiated bounded control. Neither changes answer-reveal policy | P3 instructional policy; F11 never bypasses lesson policy    |
...
| Crash while showing a gesture or checking work              | F10 cue hidden/no completion inferred; an F11 run cancels and no native action is replayed                                            | P1/P2/P7-A/P8 |
...
## Bounded control correction record (2026-09-25)

The founder subsequently requested an exactly-two-modifier push-to-talk control:
Command+Control on macOS and Left Control+Left Alt on Windows. While held, Tro
captures speech; release finalizes rolling completed-file transcription and only
the final transcript may start one direct computer-use run. This E8 decision
supersedes F10 only inside the explicit F11 voice/text control mode. The original
F10 record remains authoritative for guided teaching and historical P1 evidence.

F11 is selected-window and fail-closed: chord-down pins one eligible external
window; the action agent cannot launch or switch apps, use shell/filesystem/
clipboard tools, or escape that identity. Normal safe actions may proceed after
release. Consequential or ambiguous actions suspend the same live run until the
main Tro window approves or rejects them. Voice cannot approve itself. Emergency
